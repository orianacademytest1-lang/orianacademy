"""
Mass update remaining course pages with modern template
This script will update Generative AI, Java, Data Analytics, and HR Training pages
"""
...
HOMEPAGE_FOOTER = '''    <footer class="footer">
        <div class="container">
...
def update_footer(filepath):
    """Replace footer in file"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Match footer starting from <footer class="footer"> to </footer>
    pattern = r'<footer class="footer">.*?</footer>'
    if re.search(pattern, content, re.DOTALL):
        content = re.sub(pattern, HOMEPAGE_FOOTER.replace('    <footer', '<footer').strip(), content, flags=re.DOTALL)
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        return True
    return False

# Update footers for remaining pages
course_dir = r'c:\Users\karth\.gemini\antigravity\scratch\oriana-academy\courses'
courses = ['generative-ai.html', 'java.html', 'data-analytics.html', 'hr-training.html']

for course in courses:
    filepath = os.path.join(course_dir, course)
    if os.path.exists(filepath):
        success = update_footer(filepath)
        print(f"{'✓' if success else '✗'} Footer updated: {course}")
    else:
